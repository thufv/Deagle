...
bool gcc_modet::doit()
{
  ...
  {
  case compilet::LINK_LIBRARY: debugx("Linking a library only"); break;
  case compilet::COMPILE_ONLY: debugx("Compiling only"); break;
  case compilet::ASSEMBLE_ONLY: debugx("Assembling only"); break;
  case compilet::PREPROCESS_ONLY: debugx("Preprocessing only"); break;
  case compilet::COMPILE_LINK: debugx("Compiling and linking a library"); break;
  case compilet::COMPILE_LINK_EXECUTABLE: debugx("Compiling and linking an executable"); break;
     ...
     cmdline.isset("winx64"))
  {
    ...
    debugx("Enabling Visual Studio syntax");
  }
  else
...
int gcc_modet::gcc_hybrid_binary()
{
  ...
  if(act_as_ld)
    debugx("Running ld to generate hybrid binary");
  else
