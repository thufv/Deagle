...
\*******************************************************************/
std::map<exprt, symbolt> faild_symbols;
  ...
  const modet mode)
{
  ...
  if(may_fail)
  {
    ...
    else if (faild_symbols.find(pointer) != faild_symbols.end())
    {
        failure_value=faild_symbols[pointer].symbol_expr();
        failure_value.set(ID_C_invalid_object, true);
    }
    else
    {
