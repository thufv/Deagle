...
// __SZH_ADD_BEGIN__
  if(cmdline.isset("closure"))
  {
    options.set_option("closure", true), version_set = true;
    if (!version_set)
        options.set_option("smt2", true), version_set = true;
  }

  if(cmdline.isset("icd"))
  {
    options.set_option("icd", true), version_set = true;
    if (!version_set)
        options.set_option("smt2", true), version_set = true;
  }
// __SZH_ADD_END__
...
int cbmc_parseoptionst::doit()
{
  ...
  std::cout << "Deagle version 1.3 64-bit linux\n\n";
  ...
  {
      std::cout << "Please input a file to verify.\n";
      return 0;
  }
