...
{
  message.debugx()<<"$fresh#"+i2string(uniq)<<messaget::eom;
...
/* pointer */
  ...
  message.debugx()<<"delay_read: " << messaget::eom;
  ...
  const unsigned current_thread)
{
  ...
  message.debugx() << "write: " << object << messaget::eom;
  ...
  const unsigned current_thread)
{
  ...
  // wrote in the buffer.
  message.debugx() << "det flush: " << messaget::eom;
  const varst &vars=(*this)(object);
  ...
  const bool tso_pso_rmo) // true: tso/pso/rmo; false: power
{
  ...
  message.debugx() << "nondet flush: " << object << messaget::eom;
  ...
  Forall_goto_functions(f_it, goto_functions)
    Forall_goto_program_instructions(i_it, f_it->second.body)
    {
        ...
        forall_rw_set_w_entries(w_it, rw_set)
          ...
          {
            message.debugx() <<"debug: "<<id2string(w_it->second.object)
...
{
  shared_buffers.message.debugx() << "visit function "<<function << messaget::eom;
  ...
  Forall_goto_program_instructions(i_it, goto_program)
  {
    ...
    shared_buffers.message.debugx() << "instruction "<<instruction.type << messaget::eom;
    ...
    if(instruction.is_assign())
    {
        try
        {
          ...
          forall_rw_set_r_entries(e_it, rw_set)
            if(shared_buffers.is_buffered(ns, e_it->second.symbol_expr,false))
            {
              shared_buffers.message.debugx() << "flush restore: " 
