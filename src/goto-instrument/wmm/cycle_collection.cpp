  ...
  std::set<critical_cyclet>& set_of_cycles)
{
  ...
  for(std::set<unsigned>::const_iterator it=thin_air_events.begin();
    ...
    ++it)
    egraph.message.debugx()<<egraph[*it]<<";";

  egraph.message.debugx() << messaget::eom;
#endif
}
  ...
  memory_modelt model)
{
    ...
    st_it!=order->end(); ++st_it)
  {
    ...
    egraph.message.debugx() << "explore " << egraph[source].id << messaget::eom;
  ...
  unsigned number)
{
  ...
  while(!stack.empty())
  {
    ...
    egraph.message.debugx() << "extract: " << egraph[current_vertex].get_operation() 
...
{
#ifdef DEBUG
  for(unsigned i=0; i<80; egraph.message.debugx() << "-", ++i);
  egraph.message.debugx() << messaget::eom;
  egraph.message.debugx() << "marked size:" << marked_stack.size() 
    << messaget::eom;
  std::stack<unsigned> tmp;
  while(!point_stack.empty())
  {
    egraph.message.debugx() << point_stack.top() << " | ";
  ...
  while(!marked_stack.empty())
  {
    egraph.message.debugx() << marked_stack.top() << " | ";
  ...
  egraph.message.debugx() << "bcktck "<<egraph[vertex].id<<"#"<<vertex<<", "
    ...
    if(!get_com_only)
    {
        ...
        w_it!=egraph.po_out(vertex).end(); w_it++)
      {
            ...
            || this_vertex.unsafe_pair(egraph[source],model)) )
        {
          ...
          {
            egraph.message.debugx() << new_cycle.print_name(model,false) 
              << messaget::eom;
