...
bool inline instrumentert::local(const irep_idt& id)
{
  ...
  {
    message.debugx()<<"Exception: "<<exception << messaget::eom;
    return false;
  }
}
    ...
    std::set<instrumentert::cfg_visitort::nodet>& ending_vertex)
{
  ...
  instrumenter.message.debugx() << "visit function "<<function << messaget::eom;
    ...
    instrumenter.goto_functions.function_map[function].body)
  {
    ...
    instrumenter.message.debugx() << "visit instruction "<<instruction.type
      << messaget::eom;
...
)
{
  ...
  {
    instrumenter.message.debugx() << "backward goto" << messaget::eom;          
  ...
  for(target_sett::const_iterator prev=instruction.incoming_edges.begin();
    ...
    if(in_pos.find(*prev)!=in_pos.end())
      for(std::set<nodet>::const_iterator s_it=in_pos[*prev].begin();
        s_it!=in_pos[*prev].end(); ++s_it)
      {
        instrumenter.message.debugx() << s_it->first<<"-po->"<<new_fence_node
          << messaget::eom;
  ...
  )
{
  ...
  forall_rw_set_r_entries(r_it, rw_set)
  {
    ...
    if(ref.find(&r_it->second)!=ref.end())
    {
      ...
      instrumenter.message.debugx() << "sh: (through "
      ...
      instrumenter.message.debugx() << "sh: "<<id2string(r_it->second.object)
    ...
    instrumenter.message.debugx() << "new Read"<<read<<" @thread"
      ...
      ++prev)
    {
      if(in_pos.find(*prev)!=in_pos.end())
        for(std::set<nodet>::const_iterator s_it=in_pos[*prev].begin();
          ...
          ++s_it)
        {
           instrumenter.message.debugx() << s_it->first<<"-po->"
    ...
    for(id2nodet::iterator id_it=with_same_var.first;
      ...
      {
        instrumenter.message.debugx() << id_it->second<<"<-com->"
          <<new_read_node << messaget::eom;
  ...
  forall_rw_set_w_entries(w_it, rw_set)
  {
    ...
    if(ref.find(&w_it->second)!=ref.end())
    {
      ...
      instrumenter.message.debugx() << "sh: "<<id2string(w_it->second.object)
    ...
    instrumenter.message.debugx() << "new Write "<<write<<" @thread"<<(thread)
    ...
    {
      instrumenter.message.debugx() << previous<<"-po->"<<new_write_node
        << messaget::eom;
    ...
    for(id2nodet::iterator idr_it=r_with_same_var.first;
      ...
      {
        instrumenter.message.debugx() <<idr_it->second<<"<-com->" 
          <<new_write_node << messaget::eom;
        std::map<unsigned,unsigned>::const_iterator entry=
    ...
    for(id2nodet::iterator idw_it=w_with_same_var.first;
      ...
      {
        instrumenter.message.debugx() << idw_it->second<<"<-com->"
          <<new_write_node << messaget::eom;
        std::map<unsigned,unsigned>::const_iterator entry=
  ...
  if(!no_dependencies)
  {
    forall_rw_set_w_entries(write_it, rw_set)
      forall_rw_set_r_entries(read_it, rw_set)
      {
        ...
        instrumenter.message.debugx() << "dp: Write:"<<write<<"; Read:"<<read
          << messaget::eom;
...
{
  message.debugx() << "spurious by CFG? " << messaget::eom;
  goto_programt interleaving;
  ...
  message.debugx() << "CFG:"<<is_spurious << messaget::eom;
  return is_spurious;
  #else
  ...
  bool hide_internals)
{
  ...
  {
#ifdef PRINT_UNSAFES
    message.debugx() << it->print_unsafes() << messaget::eom;
#endif
...
void instrumentert::print_outputs(memory_modelt model, bool hide_internals)
{
  ...
  else if(num_sccs!=0)
  {
    ...
    message.debugx() << "no cycles to output" << messaget::eom;
