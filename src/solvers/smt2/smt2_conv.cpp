...
/*
 * union_flag:
 *          bv operations like bvand bvor is imcompatible between array and bv. i.e.
 *              array.0 is a array with 40 elements which has 8 bits, so array.0 is total 320 bits
 *              (declare-fun array.0 () (Array (_ BitVec 32) (_ BitVec 8)))
 *              (define-fun |c::mutex#1| () (_ BitVec 320) array.0 )
 *
 *          Definition of c::mutex#1 is illegal. So when we meet type like "c::mutex#1" (ID_union),
 *          we set union_flag = true and attain definition like:
 *              (define-fun |c::mutex#1| () (_ BitVec 320) (_ bv0 320))
 *
 *          The above definition is legal.
 */
        ...
        const struct_typet &type)
{
    ...
    if(use_datatypes)
    {
        // Structs look like:
        //  (mk-struct.top
        //  1 <component0> <component1> ... <componentN>)
        ...
        else
        {
            ...
            for(unsigned i=components.size(); i>1; i--)
            {
                ...
                if(ns.follow(op.type()).id()==ID_array)
                {
                    if(union_flag)
                    {
                        std::cout << "no need to call flatten_array; expr's id is " << op.id() << "\n";
                        convert_expr(op);
                    }
                    else
                    {
                        std::cout << "call flatten_array; expr's id is " << op.id() << "\n";
                        flatten_array(op);
                    }
                }
                else
...
void smt2_convt::convert_relation(const exprt &expr)
{
    ...
    else if(op_type.id() == ID_oc)
    {
         ...
         * PO : oclt-po  oclt-epo
	     * COE: oclt-coe
	     * RF:  oclt-rfi oclt-rfe
         * FR:  oclt-fr
         */
        std::string e1_str = id2string(to_symbol_expr(expr.op0()).get_identifier());
        std::string e2_str = id2string(to_symbol_expr(expr.op1()).get_identifier());
        ...
        {
            e1_str = e1_str.substr(0, e1_str.find_last_of('$'));
            e2_str = e2_str.substr(0, e2_str.find_last_of('$'));
//			std::cout << "====OCLT Relation: " << e1_str << " : " << e2_str << "\n";
            auto it = oclt_type_table.find(std::make_pair(e1_str, e2_str));

            if(it == oclt_type_table.end())
            {
                std::cout << "ERROR: " << e1_str << " " << e2_str << " not found\n";
                std::cout << "after adding: " << oclt_type_table.size() << "\n";
            }
            ...
            else if(it->second.second == "relaxed-po")
            {
                out << "oclt-relaxed-po ";
                convert_expr(expr.op0());
                out << " ";
...
//            out << "(bvlshr (_ bv" << power(2, array_width) - power(2, sub_width) << " " << array_width << ") ";
//            out << "distance?) ";

// this is the mask
            out << "(bvsub (_ bv" << power(2, array_width) - 1 << " " << array_width << ")";
            out << " (bvshl (_ bv" << power(2, sub_width) - 1 << " " << array_width << ") distance?)";
            out << ")"; //bvsub
...
//                std::cout << "lhs type: " << equal_expr.lhs().type().id_string() << "  "<<from_expr(ns, "", equal_expr.lhs());
//                std::cout << "  rhs type: " << equal_expr.rhs().type().id_string() << "  "<<from_expr(ns, "", equal_expr.rhs()) << "\n";
...
//                std::cout << "=========lhs: " << ns.follow(equal_expr.lhs().type()).id_string() << "\n";
                ...
                if(lhs_id == ID_struct || lhs_id == ID_union || lhs_id == ID_struct_tag || lhs_id == ID_union_tag)
                    union_flag = true; //switched to false
...
void smt2_convt::find_symbols(const exprt &expr)
{
       ...
       expr.id()==ID_nondet_symbol)
    {
        ...
        if(id.type.is_nil())
        {
            ...
            smt2_identifiers.insert(smt2_identifier);
            //// __FHY_ADD_BEGIN__
            if(smt2_identifier.find("main") != std::string::npos &&
                smt2_identifier.find("arg") != std::string::npos &&
                // __SZH_ADD_BEGIN__
                smt2_identifier.find("$wclk") == std::string::npos &&
                smt2_identifier.find("$rclk") == std::string::npos)
                // __SZH_ADD_END__
            {
                out << "; find_symbols\n";
                out << "(define-fun |"
                ...
                out << "(_ bv0 " << to_bitvector_type(expr.type()).get_width() << ")";
                out << ")" << "\n";
            }
            else
            {
...
#if 0 // not really working in any slover yet!
            out << "(assert (forall ((i ";
...
Function: smt2_convt::set_oclt_node_guard
...
\*******************************************************************/
void smt2_convt::set_oclt_node_guard()
{
    out << "\n";
    for(auto it:oclt_node_guard_map)
    {
        if(it.second.is_true())
        {
            out << "; set node guard\n";
            out << "(assert (oclt-access |";
            out << it.first << "| ";
