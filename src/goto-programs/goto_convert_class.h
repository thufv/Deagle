...
protected:
    ...
    //// __FHY_ADD_BEGIN__
    std::map<std::string, int> for_up_limit;
    static void update_for_up_limit(std::map<std::string, int>& _for_up_limit, const std::string& id , int value)
    {
        if(_for_up_limit.find(id) != _for_up_limit.end())
        {
            if(_for_up_limit[id] < value)
                _for_up_limit[id] = value;
        }
        else{
            _for_up_limit[id] = value;
        }
