...
//    std::cout << "function: " << id2string(identifier) << "\n";
    ...
    else if(identifier==CPROVER_PREFIX "assume" || identifier=="c::__VERIFIER_assume"
            || identifier == "c::assume_abort_if_not")
    {
        if(arguments.size()!=1)
        {
            err_location(function);
        ...
        //// __FHY_ADD_BEGIN__
        std::string str = from_expr(ns, "", arguments.front());
//    printf("get n scope: %s\n", str.c_str());

        std::vector<std::string> assume_args = split(str, " ");
        for(int i =0 ; i < assume_args.size(); i++)
        {
//            std::cout << assume_args[i] << "\t";
            if(assume_args[i] == "<=" || assume_args[i] == "<" )
            {
                assert(i < assume_args.size() -1);
                std::string id = assume_args[i-1];

                std::string temp_str = assume_args[i+1];
                int j = 0;
                for(; j< temp_str.length(); j++)
                    if(! (temp_str[j]>='0' && temp_str[j] <='9')){
                        break;
                    }
                temp_str = temp_str.substr(0, j);
                int value =  string2integer(temp_str).to_long();
//                std::cout << "id: " << id << "value: " << value << "\n";
                update_for_up_limit(for_up_limit, id, value);
            }
        }
