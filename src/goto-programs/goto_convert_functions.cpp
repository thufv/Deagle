...
void goto_convert_functionst::convert_function(const irep_idt &identifier)
{
  ...
  if(identifier == "c::__CPROVER_initialize")
  {
    ...
    {
      std::vector<codet> new_codes;

      auto& instructions = to_code_block(code).operands();
      for(auto& instruction: instructions)
      {
        new_codes.push_back(to_code(instruction));
      }

      std::sort(new_codes.begin(), new_codes.end(), [](const codet& a, const codet& b)
      {
        return std::atoi(a.source_location().get_line().c_str()) < std::atoi(b.source_location().get_line().c_str());
      });

      to_code_block(code).operands().clear();
      for(auto& new_code : new_codes)
        to_code_block(code).copy_to_operands(new_code);
    }
  }
  ...
  message_handlert &message_handler)
{
  ...
  try
  {
    goto_convert_functions.goto_convert();
      ...
      functions.loop_cond_ids = goto_convert_functions.loop_cond_ids;
      functions.nondet_ids = goto_convert_functions.nondet_ids;
  }
