  ...
  graphmlt &graphml)
{
  ...
  // step numbers start at 1
  std::vector<std::size_t> step_to_node(goto_trace.steps.size()+2, 0);
      ...
      it++)
  {
    ...
    graphml[node].thread_nr=it->thread_nr;
    //graphml[node].is_violation=(it->type==goto_trace_stept::ASSERT && !it->cond_value);
    graphml[node].is_violation=(it->type==goto_trace_stept::FUNCTION_CALL && it->identifier == "c::reach_error");
...
//		break;
	...
	unsigned from=step_to_node[it->step_nr];
    unsigned to = step_to_node[it->step_nr + 1];

    if(to == 0)
    {
        to = graphml.add_node();
        graphml[to].node_name="N"+i2string(to);
        graphml[to].file=graphml[from].file;
        graphml[to].line=graphml[from].line;
        graphml[to].thread_nr=graphml[from].thread_nr;
        graphml[to].is_violation = graphml[from].is_violation;
    }
