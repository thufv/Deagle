  ...
  const guardt &guard)
{
  ...
  else if(expr.operands().size() == 2)
  {
    ...
    // __SZH_ADD_BEGIN__ : only check 32-bit now
    if(type.id() == ID_signedbv && to_signedbv_type(type).get_width() == 32)
    // __SZH_ADD_END__
    {
  ...
  const guardt &guard)
{
  ...
  for(const auto &c : conditions)
  {
    // __SZH_ADD_BEGIN__ : ignore those in builtin-library functions
    std::string filename = src_expr.find_source_location().get_file().c_str();
    if(filename.find("builtin-library") != std::string::npos)
      continue;
    // __SZH_ADD_END__
